...
ownership_verify_code_contents() {
	...
	ownership_entry_count_is "$verified_code_root/lib" 14 || return 1
	...
	for owned_code_file in VERSION bin/nutmerlin \
		lib/client.sh lib/configuration.sh lib/entware.sh lib/hooks.sh lib/lifecycle.sh lib/management.sh lib/menu.sh lib/ownership.sh lib/paths.sh \
	...
	[ "$(find "$verified_code_root" -type f | wc -l)" -eq 22 ] || return 1
