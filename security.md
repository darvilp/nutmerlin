...
Installed update accepts only a local archive with a required adjacent `<archive>.sha256` sidecar and an internal allowlisted per-file checksum manifest. It verifies the archive digest, safe paths/types, inventory, file digests, and compatibility before replacing code, and it makes no network request.

The only v0.1 code downloader is a generated release-specific fresh-install launcher. It pins one HTTPS GitHub release URL and archive digest, requires the production-router gate, uses exact firmware `/usr/sbin/curl`, stages privately below `/tmp`, and verifies digest, inventory, paths, types, modes, and the internal manifest before launching the existing menu. It accepts no runtime URL/version, persists nothing itself, cleans its validated workspace on every exit, and cannot bypass menu confirmations or ownership refusal. The administrator downloads it as a named file before execution; there is no stream-to-shell path. SHA-256 supplies transfer integrity, not an independent publisher-authentication root.
